...
const columns = [
  ...
  //                 small={imageUrl.Menu + row.image}
  //                 large={imageUrl.Menu + row.image}
  ...
  // },
  {
    ...
    selector: 'card.title',
    cell: row =>  {
        if( row.card && row.card !== null){
            return <div className="fh5co-lead" > {row.card.title}</div>;
        }
        else{
        ...
        <Row>
          <Col>
            <Card className={ 'card-' + theme }>
              <CardHeader>
                <h5>
                <i className="fa fa-rss"></i> Liste des menus
                </h5>
              </CardHeader>
              <CardBody>
                <DefaultTable 
