...
const Detail = (props) => {
  ...
  let title = ( modal.row && modal.row.card !== null)? modal.row.card.title :'Aucune carte liée à ce menu, veuillez modifier le menu.';
  ...
  return (
    <div>
      <Modal isOpen={modalOpen} toggle={toggleModal} size="lg" className={"modal-"+theme}>
        <ModalHeader toggle={toggleModal}> Détail du Menu </ModalHeader>
          <ModalBody>
            {/*<FormGroup row>
              <Col md="3">
              ...
              <Col xs="12" md="9">
                { image && <ModalImage
                    small={imageUrl.Menu + image}
                    large={imageUrl.Menu + image}
