...
useEffect(() => {
  if(props.modal.show === false){
        ...
        const fetchBusinesses = () => {
          ...
          return fetch(API_URL + 'card', {method: "GET"})
          .then((res) => res.json())
          .then((data) => {
            setData(data);
          })
...
// eslint-disable-next-line
  ...
  let image = FileInput({ 
      default: modal.row && imageUrl.Menu + modal.row.image,
      maxSize: "2MB", 
...
// modal.row.image ? true : false
  ...
  const cardId = DefaultSelect(
    ...
    name:"cardId",
    selected: ( modal.row && modal.row.cardId !== null) ?
    modal.row.cardId 
    : 
    ''
    ,
  ...
  const handleSubmit = (event) => {
    ...
    Promise.resolve( props.update(`/menu/${id}`, body, true /* third param for status form data */) )
  ...
  return (
    <div>
      <Modal isOpen={modalOpen} toggle={toggleModal} size="lg" className={"modal-"+theme}>
        <ModalHeader toggle={toggleModal}> Modifier un menu </ModalHeader>
        <AvForm id="editMenu" method="post" onValidSubmit={handleSubmit}>
          <ModalBody>
          {/*<FormGroup row>
            <Col md="3">
