...
const Delete = (props) => {
  ...
  const handleSubmit = (event) => {
    Promise.resolve( props.destroy('/gallery', modal.row.id) )
  ...
  return (
    <div>
      <Modal isOpen={modalOpen} toggle={toggleModal} size="md" className={"modal-"+theme}>
        <ModalHeader toggle={toggleModal}> Supprimer la gallerie </ModalHeader>
        <AvForm id="deleteCard" method="post" onValidSubmit={handleSubmit}>
          <ModalBody>
              <h6>Voulez-vous vraiment supprimer la gallerie : <code>{ modal.row && modal.row.name }</code> ?</h6>
          </ModalBody>
        <ModalFooter>
