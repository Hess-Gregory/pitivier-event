...
const Policy = (props) => {
  ...
  React.useLayoutEffect(() => { getAll('/policy') }, [getAll]);
  ...
  const handleSubmit = (event) => {
    ...
    Promise.resolve( props.update(`/policy/${id}`, body) )
      .then(update => update.status && props.getAll('/policy'))
        ...
        <Row>
          <Col>
            <Card className={ 'card-' + theme }>
              ...
              <CardHeader>
                <h5>
                <i className="fa fa-phone"></i> Page RGPD (Policy private)
                </h5>
              </CardHeader>
              <CardBody>
                <Card>
