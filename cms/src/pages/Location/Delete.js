...
const Delete = (props) => {
  ...
  const handleSubmit = (event) => {
    Promise.resolve( props.destroy('/location/salleitem', modal.row.id) )
  ...
  return (
    <div>
      <Modal isOpen={modalOpen} toggle={toggleModal} size="md" className={"modal-"+theme}>
        <ModalHeader toggle={toggleModal}> Supprimer la salle </ModalHeader>
        <AvForm id="deleteBlog" method="post" onValidSubmit={handleSubmit}>
          <ModalBody>
              <h6>Voulez-vous vraiment supprimer la salle : <code>{ modal.row && modal.row.name }</code> ?</h6>
          </ModalBody>
        <ModalFooter>
