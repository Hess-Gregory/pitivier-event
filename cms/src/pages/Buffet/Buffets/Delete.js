...
const Delete = (props) => {
  ...
  const handleSubmit = (event) => {
    Promise.resolve( props.destroy('/buffet', modal.row.id) )
  ...
  return (
    <div>
      <Modal isOpen={modalOpen} toggle={toggleModal} size="md" className={"modal-"+theme}>
        <ModalHeader toggle={toggleModal}> Supprimer le buffet </ModalHeader>
        <AvForm id="deleteBuffet" method="post" onValidSubmit={handleSubmit}>
          <ModalBody>
              <h6>Voulez-vous vraiment supprimer le buffet : <code dangerouslySetInnerHTML={{ __html: modal.row && modal.row.name }}></code> ?</h6>
          </ModalBody>
        <ModalFooter>
