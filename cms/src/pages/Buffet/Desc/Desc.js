...
const Buffetdesc = (props) => {

  const { data, theme, getAll } = props;

  React.useLayoutEffect(() => { getAll('/buffetdesc') }, [getAll]);
  ...
  const handleSubmit = (event) => {
    ...
    Promise.resolve( props.update(`/buffetdesc/${id}`, body) )
      .then(update => update.status && props.getAll('/buffetdesc'))
        ...
        <Row>
          <Col>
            <Card className={ 'card-' + theme }>
              <CardHeader>
                <h5>
                <i className="fa fa-info"></i> Page "Buffets"
                </h5>
              </CardHeader>
              <CardBody>
...
export default connect(mapStateToProps, mapDispatchToProps)(Buffetdesc)
