...
const Testimonydesc = (props) => {
  const { data, theme, getAll } = props;
  React.useLayoutEffect(() => { getAll('/testimonydesc') }, [getAll]);
  ...
  const handleSubmit = (event) => {
    ...
    Promise.resolve( props.update(`/testimonydesc/${id}`, body) )
      .then(update => update.status && props.getAll('/testimonydesc'))
        ...
        <Row>
          <Col>
            <Card className={ 'card-' + theme }>
              <CardHeader>
                <h5>
                <i className="fa fa-info"></i> Nos témoignages (Description)
                </h5>
              </CardHeader>
              <CardBody>
...
export default connect(mapStateToProps, mapDispatchToProps)(Testimonydesc)
