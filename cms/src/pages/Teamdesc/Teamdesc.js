...
const Teamdesc = (props) => {
  ...
  React.useLayoutEffect(() => { getAll('/teamdesc') }, [getAll]);
  ...
  const handleSubmit = (event) => {
    ...
    Promise.resolve( props.update(`/teamdesc/${id}`, body) )
      .then(update => update.status && props.getAll('/teamdesc'))
        ...
        <Row>
          <Col>
            <Card className={ 'card-' + theme }>
              <CardHeader>
                <h5>
                <i className="fa fa-info"></i> Notre équipe (Description)
                </h5>
              </CardHeader>
              <CardBody>
...
export default connect(mapStateToProps, mapDispatchToProps)(Teamdesc)
