...
const Delete = (props) => {
  ...
  const handleSubmit = (event) => {
    Promise.resolve( props.destroy('/sponsor', modal.row.id) )
  ...
  return (
    <div>
      <Modal isOpen={modalOpen} toggle={toggleModal} size="md" className={"modal-"+theme}>
        <ModalHeader toggle={toggleModal}> Supprimer le sponsor </ModalHeader>
        <AvForm id="deleteCarousel" method="post" onValidSubmit={handleSubmit}>
          <ModalBody>
              <h6>Voulez-vous vraiment supprimer ce sponsor</h6>
          </ModalBody>
        <ModalFooter>
